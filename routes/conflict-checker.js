  ...
  try {
    ...
    if (req.method === 'POST' && req.body && req.body.participantEmails) {
      ...
      if (Array.isArray(req.body.participantEmails)) {
        ...
        participantEmails = req.body.participantEmails.split(',').map(email => email.trim());
      }
    } else if (req.query && req.query.participantEmails) {
      // Handle string in query parameters
      participantEmails = req.query.participantEmails.split(',').map(email => email.trim());
    }
    
    // Get time parameters
    const startTime = req.method === 'POST' ? req.body.startTime : req.query.startTime;
    const endTime = req.method === 'POST' ? req.body.endTime : req.query.endTime;
    const appointmentId = req.method === 'POST' ? req.body.appointmentId : req.query.appointmentId;
    ...
    const result = await pool.query(
      `SELECT a.id, a.title, a.start_time, a.end_time, a.participants, u.email as owner_email 
       FROM appointments a 
    ...
    console.log(`Found ${result.rows.length} potential appointments in the time range`);
    ...
    for (const appointment of result.rows) {
      ...
      // Check if owner is in our participant list
      if (participantEmails.includes(appointment.owner_email)) {
