...
IMPORTANT: When answering questions about "today", "tomorrow", or specific time periods, ONLY include appointments that match that exact timeframe. Do not show past appointments when asked about current/future dates.
...
6. Always provide the current date context and be precise about timing
      ...
      context: async (input) => {
        ...
        try {
            ...
            filteredDocs = filteredDocs.filter((doc) => {
              ...
              switch (dateFilter.type) {
                ...
                case 'tomorrow':
                  return isSameDay(appointmentDate, dateFilter.date);
                case 'week':
                  return appointmentDate >= dateFilter.start && appointmentDate <= dateFilter.end;
                default:
                  return true;
              }
            });
          ...
          return filteredDocs
            .map(
              (doc) => {
                const startTime = doc.metadata.start_time ? new Date(doc.metadata.start_time).toLocaleString() : "Unknown";
                const endTime = doc.metadata.end_time ? new Date(doc.metadata.end_time).toLocaleString() : "Unknown";
                
                return `[Appointment: ${doc.metadata.title || "Untitled"}] ${
                  doc.pageContent
                } - Start: ${startTime} - End: ${endTime} - Status: ${doc.metadata.status || "Unknown"} - Participants: ${
...
const processChat = async (message, userContext = {}) => {
  ...
  if (greetings.includes(lowerCaseMessage)) {
    return {
      response: "Hello! I can help you with your personal appointments and provide general advice about scheduling and time management. How can I assist you today?",
      hasDocuments: true,
    };
  }
...
const categorizeQuery = (message) => {
  const appointmentKeywords = [
    'appointment', 'meeting', 'schedule', 'calendar', 'book', 'cancel',
    'reschedule', 'today', 'tomorrow', 'this week', 'next week',
    'my appointments', 'my meetings', 'my schedule'
  ];
  
  const dateKeywords = [
    'today', 'tomorrow', 'this week', 'next week', 'yesterday',
    'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'
  ];
  
  const generalKeywords = [
    'how to', 'what is', 'best practices', 'tips', 'advice',
    'recommend', 'suggest', 'help me understand'
  ];
